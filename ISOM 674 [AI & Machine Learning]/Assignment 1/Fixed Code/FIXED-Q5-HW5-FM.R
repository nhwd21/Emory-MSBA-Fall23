...
# Make new data frames for standardized variables by copying unstandardized
# data frames

train_data_x_z <- train_data_x
validation_data_x_z <- validation_data_x
test_data_x_z <- test_data_x

# Overwrite each variable with the standardized variable. Use the mean and sd
# from the Training data to standardize the validation and test data.

for(i in 1:length(train_data_x)) {
  TrainMean <- mean(train_data_x[[i]])
  TrainSD <- sd(train_data_x[[i]])
  train_data_x_z[,names(train_data_x)[i]] <- (train_data_x[[i]]-TrainMean)/TrainSD
  validation_data_x_z[,names(train_data_x)[i]] <- (validation_data_x[[i]]-TrainMean)/TrainSD
  test_data_x_z[,names(train_data_x)[i]] <- (test_data_x[[i]]-TrainMean)/TrainSD
}
